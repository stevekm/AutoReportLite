...
# set the proj dir one level up, get the absolute path
ProjDir <- system("readlink -f ../",intern=TRUE)
...
# another way to exclude dirs: dir(pipelineDir,pattern = "[^report]")[file.info(dir(pipelineDir,pattern = "[^report]"))$isdir]
# optionally, don't exclude any items:
# sampleDirs<-list.dirs(path = pipelineDir,full.names = TRUE,recursive = FALSE)

SampleSheet_file<-paste0(ProjDir,"/sample-sheet.tsv")
# SampleSheet_file<-"../sample-sheet.tsv"
